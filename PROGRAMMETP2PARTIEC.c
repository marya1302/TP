   ...
   int Max(int *NB,int *NR)
{
  ...
  void Trouv_pourc (Pointeur_ATiic *R , string2 *Ca , int *N1 , int *N2 , int *Cpt1 , int *Cpt2)
    ...
    /*  POURC:=(CPT2*1111100)/CPT1; */
     ...
     void Remplir_structpartiec(Typevect_V10ATssic *T , Pointeur_ATssic *R , Typestr_Tssic *S , int *Nb , int *Nr , int *I)
    {
      /** Variables locales **/
      int N;
      int M;
   ...
   main(int argc, char *argv[])
    {
     ...
     printf ( " %s\n", "" ) ;
     printf ( " %s\n", "                                ALGORITHME ET STRUCTURES DE DONNEES DYNAMIQUES" ) ;
     printf ( " %s\n", "" ) ;
     printf ( " %s\n", "                                       TRAVAIL PRATOQUE 2 " ) ;
     printf ( " %s\n", "                                       PERIODE DU COVID 19 " ) ;
     ...
     printf ( " %s\n", "                              REALISE PAR: MITICHE SARAH MARYA ET FOUZARI RYM " ) ;
     printf ( " %s\n", "   ***************************************************************************************" ) ;
     printf ( " %s\n", "    *                             MENU PRINCIPAL                        *" ) ;
     printf ( " %s\n", "   ***************************************************************************************" ) ;
     printf ( " %s\n", "" ) ;
     printf ( " %s\n", "" ) ;
     printf ( " %s\n", "       ------->CHOISISSEZ L''UNE DES OPERATIONS SUIVANTES: " ) ;
     printf ( " %s\n", "" ) ;
     printf ( " %s\n", "    01.Créer  la  structure du tableau des arbres " ) ;
     printf ( " %s\n", "    02.afficher  la  structure du tableau des arbres " ) ;
     printf ( " %s\n", "    03.CREER  la  NOUVELLE structure globale  " ) ;
     printf ( " %s\n", "    04.afficher  la NOUVELLE  structure  " ) ;
     printf ( " %s\n", "    05.CALCULER LE POURCENTAGE " ) ;
     printf ( " %s\n", "    06.executer la partie c ." ) ;
     printf ( " %s\n", "    07.Quitter le programme." ) ;
     printf ( " %s\n", "" ) ;
     Choix  =  0 ;
     while( ( Choix != 7 )) {
       printf ( " %s", "            -VEUILLEZ ENTREE VOTRE CHOIX: " ) ;
       scanf ( " %d", &Choix ) ;
       printf ( " %d\n", Choix ) ;
       if( ( Choix == 1 )) {
       ...
       if( ( Choix == 5 )) {
         ...
         Result  =  0 ;
         printf ( " %s\n", "  == " ) ;
         printf ( " %s\n", "ENTREZ L'AGE MININMUM POUR L'INTERVALLE" ) ;
         scanf ( " %d\n", &N1 ) ;
         printf ( " %s\n", "ENTRZ L'AGE MAXIMUM POUR L'INTERVALLE" ) ;
         scanf ( " %d\n", &N2 ) ;
         printf ( " %s\n", "ENTREZ LE RESULTAT RECHERCHE (D OU R)" ) ;
         scanf ( " %s\n", Ca ) ;
         printf ( " %s\n", " LE POURCENTAGE CALCULÉ EST DE " ) ;
     ...
     Typestr_Tssic Sc;
       Tab = malloc(10 * sizeof(Pointeur_ATssic));
     for (_Izw2=0; _Izw2<10; ++_Izw2)// hadoma declaration li shekit fihom psa meme mlfo9 kayen IZW2 je pence ytbedel
       Tab[_Izw2] = malloc( sizeof(Pointeur_ATssic ));//meme hna
     Sc = malloc(sizeof(Type_Tssic));
     Sc->Champ1 = malloc(255 * sizeof(char));
     Sc->Champ2 = malloc(255 * sizeof(char));
     Sc->Champ4 = malloc(2 * sizeof(char));
      FILE*fic=fopen("resultats.txt","r");

    signed char pays[256];
    int nb_death=0;
    int nb_recovery=0;
    int i=1;
    if (fic==NULL)
    ...
    while(1)
    {
       fscanf(fic,"%s %d %d",pays ,&nb_death ,&nb_recovery);
       if (feof(fic))
         break;
          Remplir_structpartiec ( &Tab , &R1 , &Sc , &nb_death , &nb_recovery , &i );
          //AFFICHAGE
         printf("  "  );
        printf("pays : %s\n" , pays);
        printf("nbD : %d\n" , nb_death);
    ...
    fclose(fic);
       ...
       if( ( ( Choix > 7 ) || ( Choix < 1 ) )) {
         printf ( " %s\n", " -DESOLE! Votre choix est incorrecte, veuillez essayer une autre fois." ) ;
